    ...
    if (menu) {
      ...
      document.querySelectorAll('[id$="-menu"]').forEach(otherMenu => {
        if (otherMenu.id !== menu.id) {
          ...
          const otherArrow = otherMenu.previousElementSibling.querySelector('[id$="-arrow"]');
          if (otherArrow) {
      ...
      if (arrow) {
        arrow.classList.toggle('-rotate-180', !isMenuOpen);
      }
    }
  } else {
      ...
      document.querySelectorAll('[id$="-menu"]').forEach(menu => {
        if (menu.classList.contains('opacity-100') && !menu.contains(target)) {
          ...
          const arrow = menu.previousElementSibling.querySelector('[id$="-arrow"]');
          if (arrow) {
